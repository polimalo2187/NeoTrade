...
class Bot:
    ...
    async def manejar_mensajes(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # =========================
        # Releer usuario antes de validar
        # =========================
        UsuarioModel.actualizar_usuario({"telegram_id": telegram_id}, {"estado": None})  # asegurar estado
        usuario = UsuarioModel.obtener_usuario({"telegram_id": telegram_id})  # recargar usuario desde DB
        ...
        if usuario["estado"] == "esperando_api_secret":
            # Obtener API Key temporal después de recargar usuario
            usuario = UsuarioModel.obtener_usuario({"telegram_id": telegram_id})
            ...
            # Debug seguro
            # =========================
            print(f"[DEBUG] Ejecutando validar_api desde archivo: {__file__}")
            print(f"[DEBUG] API Key temporal: {api_key}, Secret largo omitido, Timestamp: {int(time.time() * 1000)}")
            ...
            if exito:
                ...
                await update.message.reply_text(
                    f"No se pudo registrar la API Key/API Secret ❌\nMotivo: {error_msg}\nIntente de nuevo. Introduzca su API Key:",
                    reply_markup=self.menu_principal()
                )
            return
