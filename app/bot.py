...
class Bot:
    ...
    def menu_principal(self):
        """
        Devuelve el teclado principal de usuario con los botones grandes (2 por fila, mitad de pantalla cada uno).
        """
        teclado = []
        # Tomamos los botones del BOTONES_PRINCIPAL en pares de 2
        for i in range(0, len(BOTONES_PRINCIPAL), 2):
            fila = BOTONES_PRINCIPAL[i:i+2]  # Dos botones por fila
            teclado_fila = [InlineKeyboardButton(text=btn, callback_data=btn) for btn in fila]
            teclado.append(teclado_fila)
        return InlineKeyboardMarkup(teclado)

    def menu_configuracion(self):
        """
        Devuelve el teclado de configuración con botones grandes (2 por fila).
        """
        teclado = []
        for i in range(0, len(BOTONES_CONFIGURACION), 2):
            fila = BOTONES_CONFIGURACION[i:i+2]
            teclado_fila = [InlineKeyboardButton(text=btn, callback_data=btn) for btn in fila]
            teclado.append(teclado_fila)
        return InlineKeyboardMarkup(teclado)
