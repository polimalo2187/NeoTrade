    ...
    async def manejar_mensajes(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if usuario["estado"] == "esperando_api_secret":
            ...
            if exito:
                ...
                await update.message.reply_text("API Key y API Secret configuradas correctamente ✅", reply_markup=self.menu_principal())
            else:
                UsuarioModel.actualizar_usuario({"telegram_id": telegram_id}, {
                    "estado": "esperando_api_key",
                    "api_key_temp": None
                })
                await update.message.reply_text("API Key o API Secret inválidas ❌. Intente de nuevo. Introduzca su API Key:", reply_markup=self.menu_principal())
            return
