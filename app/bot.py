...
class Bot:
    ...
    async def boton_click(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if query.data == "VOLVER_ATRAS":
            # Para simplificar, volver atrás muestra menú principal
            if telegram_id in ADMIN_TELEGRAM_IDS:
                await query.edit_message_text(
                    "Volviendo atrás...",
                    reply_markup=self.menu_principal_admin()
        ...
        if query.data == "🟢 Activar Bot":
            ...
            await query.edit_message_text(
                ...
                reply_markup=self.menu_principal() if telegram_id not in ADMIN_TELEGRAM_IDS else self.menu_principal_admin()
            )
            return
