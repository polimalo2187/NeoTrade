...
class Usuario:
    ...
    def validar_api(api_key, api_secret, return_error=False):
        """
        Valida la API Key y API Secret del exchange CoinW usando el endpoint actual de balances.
        return_error: si True, devuelve (exito, mensaje_error)
        """
        try:
            timestamp = str(int(time.time() * 1000))
            payload = f"access_key={api_key}&timestamp={timestamp}"
            ...
            # Endpoint actualizado de CoinW para obtener balances
            url = "https://www.coinw.com/api/v1/private?command=returnBalances"
            ...
            # Validación exitosa si se recibe diccionario de balances
            if r.status_code != 200 or not data.get("success", False):
                msg = data.get("msg", f"Error en la API: {data}")
                if return_error:
                    return False, msg
                return False
