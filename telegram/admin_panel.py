...
from exchange.wallet import WalletAdmin
...
class AdminPanel:
    ...
    async def manejar_click(self, update: Update, context: CallbackContext):
        """
        ...
        if accion == "activar_detener":
            # Aquí se podría implementar lista de usuarios para activar o detener su bot
            await query.edit_message_text("Función de activar/detener usuarios pendiente de implementación")
            ...
            await query.edit_message_text(f"💰 Capital total de todos los usuarios: {total_capital:.2f} USDT")
        elif accion == "historial_completo":
            operaciones = OperacionModel.obtener_operaciones({})
            mensaje = "\n".join([f"{o['telegram_id']}: {o.get('ganancia',0)} USDT" for o in operaciones])
            if not mensaje:
            ...
            mensaje = "\n".join([f"{r['referidor_id']}: {r.get('comision',0)} USDT" for r in referidos])
            if not mensaje:
            ...
            await query.edit_message_text("⚙️ Configuración avanzada pendiente de implementación")
        else:
